...
export default {
  ...
  data() {
    return {
      ...
      query: {
        ...
        autoInfo: null,
        version: null,
        relationTableName: 'gasfueltype_aisoandauto_mapping',
        aisoTableName: 'aiso_gas_fuel_type',
        autoTableName: 'auto_gas_fuel_type',
        pageNum: 1,
        pageSize: 10
      },
      dataAddForm: {
  ...
  methods: {
    version() {
      ...
      tab.append('tableName', 'gasfueltype_aisoandauto_mapping')
    ...
    downloadData() {
      downloadData(this.query).then(response => {
        ...
        // 配置下载的文件名
        // this.curDateTime()
        link.download = this.dataAddForm.version + '_GasFuelType-Aiso-SW.csv'
    ...
    beforeUpload(file) {
      ...
      fd.append('file', file)
      fd.append('tableName', 'gasfueltype_aisoandauto_mapping')
