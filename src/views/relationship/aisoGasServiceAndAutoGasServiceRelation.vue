...
export default {
  ...
  data() {
    return {
      ...
      query: {
        ...
        autoInfo: null,
        version: null,
        relationTableName: 'gasservice_aisoandauto_mapping',
        aisoTableName: 'aiso_gas_service',
        autoTableName: 'auto_gas_service',
        pageNum: 1,
        pageSize: 10
      },
      dataAddForm: {
  ...
  methods: {
    version() {
      ...
      tab.append('tableName', 'gasservice_aisoandauto_mapping')
    ...
    downloadData() {
      downloadData(this.query).then(response => {
        ...
        // 配置下载的文件名
        // this.curDateTime()
        link.download = this.dataAddForm.version + '_GasService-Aiso-SW.csv'
    ...
    beforeUpload(file) {
      ...
      fd.append('file', file)
      fd.append('tableName', 'gasservice_aisoandauto_mapping')
