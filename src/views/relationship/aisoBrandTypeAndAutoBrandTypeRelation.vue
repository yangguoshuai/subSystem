...
<script>
import { pageBrand as page, insertData, updateData, deleteData, downloadDataBrand as downloadData, uploadFile, getVersion, pageAiso as pageAisoGas } from '@/api/relationship/AisoAndSwMappingAll'
...
export default {
  ...
  data() {
    return {
      ...
      dataAddForm: {
        ...
        autoID: null,
        version: null,
        relationTableName: 'gasbrand_aiso_and_auto_mapping',
        autoTableName: 'auto_gas_brand'
      },
      // aiso查询的对象
      aisoQuery: {
        customID: undefined,
      ...
      // 弹出框的 title 文本
      textMap: {
        add: '添加aisoGasBrand数据',
        update: '更新aisoGasBrand数据',
        aisoGas: 'aisoGas数据',
        autoGas: 'autoGas数据'
      }
    }
  ...
  methods: {
    version() {
      ...
      tab.append('tableName', 'gasbrand_aiso_and_auto_mapping')
    ...
    downloadData() {
      downloadData(this.query).then(response => {
        ...
        // 配置下载的文件名
        // this.curDateTime()
        link.download = this.dataAddForm.version + '_GasBrand-Aiso-SW.csv'
    ...
    beforeUpload(file) {
      ...
      fd.append('file', file)
      fd.append('tableName', 'gasbrand_aiso_and_auto_mapping')
