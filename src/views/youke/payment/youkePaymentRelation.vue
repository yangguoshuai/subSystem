...
<script>
import { page, pageAiso, pageInstitute, addAisoInstituteData, delObj, downloadData, uploadFile } from '@/api/youke/payment/youkePaymentRelation'
export default {
  ...
  methods: {
    ...
    downloadData(scheduleType) {
      downloadData().then(response => {
        ...
        link.download = 'youke_aiso_payment_relation.csv'
        link.click()
      })
    },
    // 上传文件
