<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      ...
      <el-col :span="4">
        <span>中文名称:</span>
        <el-input v-model="query.cnInfo" clearable style="width: 200px;" placeholder="模糊查询" @keyup.enter.native="handleAutoGasBrandQueryFilter" @input="handleAutoGasBrandQueryFilter" />
      </el-col>
      <el-col :span="4">
        <span>英文名称:</span>
        <el-input v-model="query.enInfo" clearable style="width: 200px;" placeholder="模糊查询" @keyup.enter.native="handleAutoGasBrandQueryFilter" @input="handleAutoGasBrandQueryFilter" />
      </el-col>
      <el-col :span="4">
        <span>id：</span>
        <el-input v-model="query.autoDataId" clearable style="width: 200px;" placeholder="查询" @keyup.enter.native="handleAutoGasBrandQueryFilter" @input="handleAutoGasBrandQueryFilter" />
      </el-col>
      <div class="filter-container">
      ...
      <template>
        <div class="addAlias-div" style="padding-top: 20px;">
          <el-form ref="dataAddForm" :model="dataAddForm" :rules="addFormRules" label-width="150px">
            <el-form-item label="ID" prop="autoDataId" width="300">
              <el-col :span="12">
                <el-input v-model="dataAddForm.autoDataId" clearable placeholder="输入ID" />
              </el-col>
            </el-form-item>
            <el-form-item label="请输入中文名称" prop="cnInfo" >
              <el-col :span="12">
...
export default {
  ...
  data() {
    return {
      ...
      textMap: {
        add: '添加Auto加油站品牌数据',
        update: '更新Auto加油站品牌数据'
      },
      addFormRules: {
        autoDataId: [
          { required: true, message: '请填入数字或字母' },
          { validator(rule, value, callback, source, options) {
            var errors = []
            if (!/^[0-9a-zA-Z]+$/.test(value)) {
              callback('输入值必须为数字或字母')
            }
            callback(errors)
          }
          }
  ...
  methods: {
    version() {
      ...
      tab.append('tableName', 'auto_gas_brand')
    ...
    downloadData() {
      downloadData(this.query).then(response => {
        ...
        // 配置下载的文件名
        // this.curDateTime()
        link.download = this.dataAddForm.version + '_GasBrand-SW.csv'
    ...
    beforeUpload(file) {
      ...
      fd.append('file', file)
      fd.append('tableName', 'auto_gas_brand')
