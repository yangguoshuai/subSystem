  ...
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-col :span="5">
        ...
        <el-input v-model="query.autoaiName" clearable style="width: 130px;" placeholder="autoaiName" @keyup.enter.native="handleAisoInstituteQueryFilter" @input="handleAisoInstituteQueryFilter" />
      </el-col>
      <el-col :span="4">
        <span>aisoName: </span>
        <el-input v-model="query.aisoName" clearable style="width: 130px;" placeholder="aisoName" @keyup.enter.native="handleAisoInstituteQueryFilter" @input="handleAisoInstituteQueryFilter" />
      </el-col>
      <el-col :span="4">
        <span>category: </span>
        <el-select v-model="query.category" clearable placeholder="category" style="width: 130px;" @change="chooseCategory">
          <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value"/>
        </el-select>
      </el-col>
      <el-col :span="4">
        <span>source: </span>
        <el-select v-model="query.source" clearable placeholder="source" style="width: 130px;" @change="chooseSource">
          <el-option
    ...
    <el-table v-loading.body="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%">
      <el-table-column prop="autoaiCode" label="autoaiCode" min-width="45%">
        <template slot-scope="scope">
          <span>{{ scope.row.autoaiCode }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="autoaiName" label="autoaiName" min-width="45%">
        <template slot-scope="scope">
      ...
      <el-table-column prop="aisoName" label="aisoName" min-width="45%">
        <template slot-scope="scope">
          <span>{{ scope.row.aisoName }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="aisoCategory" label="aisoCategory" min-width="45%">
        <template slot-scope="scope">
        ...
        <div class="addAlias-div" style="padding-top: 20px;">
          <el-form :rules="addFormRules" label-width="150px">
            <el-form-item label="aiso数据" width="300">
              <el-col :span="12">
                <el-input v-model="dataAddForm.aiso.name" clearable placeholder="aiso数据" @focus="chooseAiso()" />
              </el-col>
            </el-form-item>
            <el-form-item label="autoai数据" >
              <el-col :span="12">
                <el-input v-model="dataAddForm.institute.name" clearable placeholder="autoai数据" @focus="chooseInstitute()"/>
              </el-col>
            </el-form-item>
          </el-form>
          <el-col :span="15">
    ...
    <el-dialog :title="textMap.institutetitle" :visible.sync="instituteDialogVisible" :before-close="handleClose" lock-scroll style="overflow: hidden;padding:10px 20px 30px 20px;">
      ...
      <el-table v-loading.body="instituteListLoading" :key="tableKey" :data="instituteQuery.list" fit highlight-current-row stripe min-height="300" max-height="500" size="mini" >
        <el-table-column label="序号" type="index" show-overflow-tooltip width="50"/>
        <el-table-column prop="name" label="autoaiName" >
          <template slot-scope="scope">
            <span>{{ scope.row.name }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="code" label="autoaiCode" >
          <template slot-scope="scope">
            <span>{{ scope.row.code }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="autoaiCode" prop="autoaiCode">
          <el-input v-model="form.autoaiCode" :disabled="true"/>
        </el-form-item>
        <el-form-item label="autoaiName" prop="autoaiName">
          <el-input v-model="form.autoaiName" placeholder="请输入autoaiName"/>
        </el-form-item>
...
<script>
import { page, pageAiso, pageInstitute, addAisoInstituteData, downloadAll, delObj, getObj, addObj, uploadExcel } from '@/api/type/aisoAutoaiData'
...
export default {
  ...
  methods: {
    ...
    makeSureAdd() {
      if (this.dataAddForm.aiso.name === null || this.dataAddForm.institute.name === null) {
        this.$message({
          showClose: true,
          message: '请选择数据',
          type: 'info'
    ...
    downloadData(scheduleType) {
      ...
      downloadAll().then(response => {
        ...
        // 配置下载的文件名
        // this.curDateTime()
        link.download = 'aiso-autoai-data.csv'
    ...
    handleNameFilter() {
      ...
      selectByName(this.search_name, this.category, this.search_city, 1).then(response => {
        if (response.data.code === 200) {
