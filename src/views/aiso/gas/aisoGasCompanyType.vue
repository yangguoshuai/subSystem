  ...
  data() {
    return {
      ...
      query: {
        ...
        pageNum: 1,
        pageSize: 10,
        tableName: 'aiso_gas_company_type'
      },
      dataAddForm: {
        id: null,
        dataId: null,
      ...
      // 弹出框的 title 文本
      textMap: {
        add: '添加aisoGasCompanyType数据',
        update: '更新aisoGasCompanyType数据'
      },
      addFormRules: {
        dataId: [
  ...
  methods: {
    ...
    downloadData() {
      downloadData(this.query).then(response => {
        ...
        // 配置下载的文件名
        // this.curDateTime()
        link.download = 'aiso-gas-comPanyType.csv'
    ...
    beforeUpload(file) {
      ...
      fd.append('file', file)
      fd.append('tableName', 'aiso_gas_company_type')
