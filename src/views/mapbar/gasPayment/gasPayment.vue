...
<script>
import { page, insertData, updateData, downloadData, uploadFile } from '@/api/mapbar/gasPayment/gasPayment'
export default {
  ...
  data() {
    return {
      ...
      // 弹出框的 title 文本
      textMap: {
        add: '添加研究院GasPayment数据',
        update: '更新研究院GasPayment数据'
      },
      addFormRules: {
        cnId: [
  ...
  methods: {
    ...
    downloadData(scheduleType) {
      downloadData().then(response => {
        ...
        link.download = 'mapbar_gas_payment.csv'
        link.click()
      })
    },
    // 上传文件
