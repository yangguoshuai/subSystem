    ...
    <div class="filter-container">
      ...
      <div>
        <el-upload
          ...
          multiple
          accept=".txt,"
          style="margin-left: 1080px;    margin-top: -2.7%;">
...
<script>
import { page, insertData, updateData, downloadData, uploadFile } from '@/api/mapbar/gasFuelType/gasFuelType'
export default {
  ...
  data() {
    return {
      ...
      // 弹出框的 title 文本
      textMap: {
        add: '添加研究院GasFuelType数据',
        update: '更新研究院GasFuelType数据'
      },
      addFormRules: {
        cnId: [
          { required: true, message: '请填入ID' }
        ],
        cnInfo: [
          {
            required: true,
  ...
  methods: {
    ...
    downloadData(scheduleType) {
      downloadData().then(response => {
        ...
        link.download = 'mapbar_gas_fuel_type.csv'
        link.click()
      })
    },
    // 上传文件
