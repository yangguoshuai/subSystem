...
<script>
import { page, insertData, updateData, downloadData, uploadFile } from '@/api/mapbar/gasService/gasService'
export default {
  ...
  data() {
    return {
      ...
      // 弹出框的 title 文本
      textMap: {
        add: '添加研究院GasService数据',
        update: '更新研究院GasService数据'
      },
      addFormRules: {
        cnId: [
  ...
  methods: {
    ...
    downloadData(scheduleType) {
      downloadData().then(response => {
        ...
        link.download = 'mapbar_gas_service.csv'
        link.click()
      })
    },
    // 上传文件
