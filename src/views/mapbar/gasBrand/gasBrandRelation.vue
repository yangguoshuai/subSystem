<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <el-col :span="6">
        <span>研究院名称: </span>
        <el-input v-model="query.mapbarName" clearable style="width: 200px;" @keyup.enter.native="handleQueryFilter" @input="handleQueryFilter" />
      </el-col>
      <el-col :span="6">
        <span>aiso名称: </span>
    ...
    <el-table v-loading.body="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%">
      <el-table-column prop="mapbarId" label="研究院编号" min-width="45%">
        <template slot-scope="scope">
          <span>{{ scope.row.mapbarId }}</span>
        </template>
      </el-table-column>
      <el-table-column prop="mapbarName" label="研究院名称" min-width="45%">
        <template slot-scope="scope">
          <span>{{ scope.row.mapbarName }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap.aisotitle" :visible.sync="aisoDialogVisible" :before-close="handleAisoClose" lock-scroll style="overflow: hidden;padding:10px 20px 30px 20px;">
      ...
      <el-table v-loading.body="aisoListLoading" :key="tableKey" :data="aisoQuery.list" fit highlight-current-row stripe min-height="300" max-height="500" size="mini" >
        <el-table-column prop="code" label="aiso编号" >
          <template slot-scope="scope">
    ...
    <el-dialog :title="textMap.institutetitle" :visible.sync="instituteDialogVisible" :before-close="handleClose" lock-scroll style="overflow: hidden;padding:10px 20px 30px 20px;">
      ...
      <el-table v-loading.body="instituteListLoading" :key="tableKey" :data="instituteQuery.list" fit highlight-current-row stripe min-height="300" max-height="500" size="mini" >
        <el-table-column prop="cnId" label="研究院编号" >
          <template slot-scope="scope">
            <span>{{ scope.row.cnId }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="cnInfo" label="研究院名称" >
          <template slot-scope="scope">
...
<script>
import { page, pageAiso, pageInstitute, addAisoInstituteData, delObj, downloadData, uploadFile } from '@/api/mapbar/gasBrand/gasBrandRelation'
export default {
  ...
  methods: {
    ...
    handleAisoChoice(row) {
      this.dataAddForm.aiso.dataId = row.code
      this.dataAddForm.aiso.cnInfo = row.name
      // 关闭类型弹框
    ...
    downloadData(scheduleType) {
      downloadData().then(response => {
        ...
        link.download = 'mapbar_aiso_gas_brand_relation.csv'
        link.click()
      })
    },
    // 上传文件
